...
IMAGE imgChoicebar;//选项版
IMAGE imgContinue;//继续
IMAGE imgFileKeep;//存档
...
void gameInit()//游戏初始化
{
	...
	loadimage(&imgChoicebar, "res/啦啦啦/11.jpg");
	loadimage(&imgContinue, "res/啦啦啦/21(3).png");
	loadimage(&imgFileKeep, "res/啦啦啦/21(1).png");
	//初始化
...
void userClick()//用户操作
{
	...
	if (peekmessage(&msg))//判断当前有没有消息
	{
		if (msg.message == WM_LBUTTONDOWN)//鼠标左键按下
		{
			...
			else if (msg.x>44&&msg.x<164&&msg.y>0&&msg.y<60) {
				while (1) {
					putimagePNG(300, 200, &imgChoicebar);
				}
			}
			else//收集阳光
			{
